...
import s3p.data.stackforum.StackForumDaily;
import s3p.data.stackforum.StackForumHourly;
import s3p.data.stackforum.StackForumWeekly;
import s3p.data.storage.table.JobEntity;
import s3p.data.utils.AnomalyDetectionUtils;
import s3p.data.utils.TableUtils;

public class StackForumTotalJob {
	...
	public static void run(S3PDataConfig config, String platform) {
		...
		DateTime cur = new DateTime(job.getLast()).minusDays(1);
		while (cur.lteq(end)) {
